...
<template>
  <UDashboardGroup v-if="template && progress && activeItem" storage-key="program-sidebar">
    <UDashboardSidebar :ui="{ header: 'px-4 pt-4 pb-0', body: 'px-4 py-3' }">
      <template #header>
        <NuxtLink :to="`/learn/${template.id}`" class="text-sm text-primary">
          {{ t('program.viewer.exit.label') }}
        </NuxtLink>
        <h3 class="mt-2 font-heading font-bold text-highlighted">{{ template.title }}</h3>
    ...
    <UDashboardPanel :ui="{ body: 'p-0 gap-0 overflow-y-auto' }">
      <template #body>
